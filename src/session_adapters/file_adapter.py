...
class FileAdapter(AbstractAdapter[_FileRequest]):
    ...
    def parse_request(
        ...
        if url_parts.netloc and url_parts.netloc != "localhost":
            raise TypeError("URLs with hostname components are not allowed, 'localhost' supported only")

        return _FileRequest(
            path=Path(str(url_parts.path)),
        ...
        try:
            if request.path.exists():
                ...
                if request.path.is_file:
                    ...
                    response.send_header(HTTPHeader.CONTENT_LENGTH, str(request.path.stat().st_size))
                else:
                    # TODO file listing
